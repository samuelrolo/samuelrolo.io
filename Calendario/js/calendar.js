...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Esta parte pode ser simplificada ou removida se booking.js já a cobre completamente
    // e se window.openModalGlobal for usado consistentemente.
    ...
    const openTermsModalCoachingLink = document.getElementById("open-terms-modal-coaching"); // Pode ser tratado por booking.js
    const openTermsModalCvLink = document.getElementById("open-terms-modal-cv");       // Pode ser tratado por booking.js

    function openModalHelper() { 
        if (typeof window.openModalGlobal === 'function') {
            window.openModalGlobal(window.currentFlow);
        } else if (termsModal) {
             termsModal.style.display = "block"; 
        }
    }
    function closeModalHelper() { if (termsModal) termsModal.style.display = "none"; }

    // Estes listeners podem ser redundantes se booking.js já os configura após a inicialização de cada fluxo.
    // Considere se são necessários aqui ou se a lógica em booking.js é suficiente.
    // if (openTermsModalCoachingLink) openTermsModalCoachingLink.addEventListener('click', (e) => { e.preventDefault(); window.currentFlow = 'coaching'; openModalHelper(); });
    // if (openTermsModalCvLink) openTermsModalCvLink.addEventListener('click', (e) => { e.preventDefault(); window.currentFlow = 'cv'; openModalHelper(); });
