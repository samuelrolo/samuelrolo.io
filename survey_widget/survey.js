document.addEventListener("DOMContentLoaded", function() {
    ...
    if (!surveyWidgetRoot) {
        console.error("[SurveyLOG] Survey widget root container (.survey-container) not found. Cannot initialize survey.");
        return;
    }
    ...
    function updateQuestionView() {
        ...
        questionContainers.forEach((container, index) => {
            if (container && container.style) { // Added check for container and container.style
                container.style.display = index === currentQuestionIndex ? "block" : "none";
            } else {
                console.error(`[SurveyLOG] Error: Question container at index ${index} is null or has no style property.`);
            }
        });
    ...
    function calculateArchetype() {
        ...
        if (dominantArchetype === "" && maxCount > 0) { 
