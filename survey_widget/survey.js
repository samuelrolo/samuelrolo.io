document.addEventListener("DOMContentLoaded", function() {
    const surveyWidgetRoot = document.querySelector(".survey-container");
    if (!surveyWidgetRoot) {
        console.error("Survey widget root container (.survey-container) not found. Cannot initialize survey.");
        return;
    }

    // Elementos do DOM (alguns escopados ao widget, IDs podem ser globais mas é boa prática escopar se possível)
    const questionContainers = surveyWidgetRoot.querySelectorAll(".question-container");
    const resultsContainer = document.getElementById("results-container"); 
    const surveyContentContainer = document.getElementById("survey-content-container");
    const progressFill = surveyWidgetRoot.querySelector(".progress-fill"); 
    ...
    console.log("[SurveyLOG] Initializing. Total questions found within .survey-container:", totalQuestions);
    const userAnswers = {};
    ...
    function init() {
        console.log("[SurveyLOG] init() called. totalQuestions:", totalQuestions);
        if (totalQuestions === 0) {
            console.error("[SurveyLOG] No question containers found. Survey cannot start.");
            ...
            if (resultsContainer) {
                resultsContainer.innerHTML = "<p>Erro: Nenhuma pergunta encontrada para o questionário.</p>";
    ...
    function attachEventListeners() {
        questionContainers.forEach((container, index) => {
            ...
            options.forEach(option => {
                option.addEventListener("change", () => {
                    ...
                    console.log(`[SurveyLOG] Answered q${index}: ${option.value}`);
                    updateButtonStates();
                });
            });
        });
        ...
        if(prevBtn) prevBtn.addEventListener("click", () => {
            console.log("[SurveyLOG] Prev button clicked.");
        ...
        if(nextBtn) nextBtn.addEventListener("click", () => {
            console.log(`[SurveyLOG] Next/Submit button clicked. Current index: ${currentQuestionIndex}, Answered: ${userAnswers[currentQuestionIndex]}`);
            if (userAnswers[currentQuestionIndex] === undefined && currentQuestionIndex < totalQuestions - 1) {
                console.log("[SurveyLOG] Answer required to proceed.");
                // Poderia adicionar um alerta visual para o utilizador aqui
                return;
            }
            ...
            if (currentQuestionIndex < totalQuestions - 1) {
                currentQuestionIndex++;
                updateQuestionView();
            } else { // É a última pergunta ou o botão "Ver Resultado"
                if (userAnswers[currentQuestionIndex] !== undefined) {
                    console.log("[SurveyLOG] Last question answered. Displaying results.");
                    displayResults();
                } else {
                    console.log("[SurveyLOG] Last question not answered. Cannot display results.");
                     // Poderia adicionar um alerta visual para o utilizador aqui
                }
            }
        });
    ...
    function updateQuestionView() {
        console.log(`[SurveyLOG] updateQuestionView for index: ${currentQuestionIndex}`);
        questionContainers.forEach((container, index) => {
    ...
    function updateButtonStates() {
        console.log(`[SurveyLOG] updateButtonStates: current=${currentQuestionIndex}, total=${totalQuestions}`);
        if(prevBtn) prevBtn.disabled = currentQuestionIndex === 0;
        if(nextBtn) {
            if (totalQuestions > 0 && currentQuestionIndex === totalQuestions - 1) {
                console.log("[SurveyLOG] Last question. Setting button to 'Ver Resultado'.");
                nextBtn.textContent = "Ver Resultado";
                nextBtn.disabled = userAnswers[currentQuestionIndex] === undefined;
            } else {
                console.log("[SurveyLOG] Not last question. Setting button to 'Próxima'.");
                nextBtn.textContent = "Próxima";
                nextBtn.disabled = (totalQuestions > 0 && userAnswers[currentQuestionIndex] === undefined);
            }
            console.log(`[SurveyLOG] Button text: ${nextBtn.textContent}, Disabled: ${nextBtn.disabled}`);
        }
    }
    ...
    function calculateArchetype() {
        ...
        console.log("[SurveyLOG] Answer counts:", counts);

        let maxCount = 0;
        let dominantArchetype = "";
        // Prioridade para desempate: A > B > C > D (ou a primeira encontrada com maxCount)
        const archetypeOrder = ["A", "B", "C", "D"];
        for (const type of archetypeOrder) {
            if (counts.hasOwnProperty(type) && counts[type] > maxCount) {
                maxCount = counts[type];
                dominantArchetype = type;
            }
        }
        // Se ainda houver empate (ex: todas 0, ou várias com o mesmo maxCount > 0)
        if (dominantArchetype === "" && maxCount > 0) { // Se maxCount > 0 mas dominantArchetype não foi setado (improvável com a lógica acima)
             dominantArchetype = archetypeOrder.find(type => counts[type] === maxCount) || "A";
        } else if (dominantArchetype === "") { // Todas as contagens são 0 ou não há respostas
            dominantArchetype = "A"; // Default para evitar erro, ou pode ser tratado como erro
            console.log("[SurveyLOG] No dominant archetype found or no answers, defaulting.");
        }
        console.log("[SurveyLOG] Dominant archetype key:", dominantArchetype);
        return dominantArchetype;
    }

    function displayResults() {
        console.log("[SurveyLOG] displayResults() called.");
        if (!resultsContainer || !surveyContentContainer) {
            console.error("[SurveyLOG] Results container or survey content container not found.");
            return;
        }
